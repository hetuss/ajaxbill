          ...
          <table class="table table-bordered" id="laravel_crud">
           ...
           <tbody id="posts-crud">
              ...
              <tr id="post_id_{{ $post->id }}">
                 ...
                 <td>{{ $post->body }}</td>
                <td>
                 <button type="submit" name="submit" class="btn btn-info"><a href="{{route('detailspage',$post->id)}}">Detail</a></button>
 ...
 if ($("#postForm").length > 0) {
     ...
     submitHandler: function(form) {
      ...
      $.ajax({
          ...
          dataType: 'json',
          success: function (data) {
              var post = '<tr id="post_id_' + data.id + '"><td>' + data.id + '</td><td>' + data.title + '</td><td>' + data.email + '</td><td>' + data.body + '</td>';
